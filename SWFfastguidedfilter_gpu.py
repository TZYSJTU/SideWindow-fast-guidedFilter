...
if __name__ == '__main__':
    ...
    lr_output.requires_grad = True
    hr_guide.requires_grad = True
    lr_guide.requires_grad = True
    res = s.forward(lr_guide.cuda(), lr_output.cuda() , hr_guide.cuda())
    res = res.cpu().detach()
    image1 = Image.open('hr_hdr.jpg')
    image2 = np.transpose(np.squeeze(res.numpy()), (1,2,0))
